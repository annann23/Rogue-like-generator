...
export async function generateSurveyQuestions(totalRuns = 0): Promise<SurveyQuestionsResponse> {
  // 현재 시각(초) + 플레이 횟수를 조합한 시드 → 새로고침마다 다른 값 보장
  const timeSec = Math.floor(Date.now() / 1000);
  const questionSeed = `${timeSec}-run${totalRuns}`;
...
세션 코드: ${questionSeed}
→ 이 코드가 다르면 반드시 완전히 다른 주제 조합으로 질문을 생성할 것. 이전 세션과 같은 질문 반복 금지.
...
5. 아래 주제는 절대 금지 (개인정보 또는 너무 자주 나옴):
   ...
   - 타인의 이름 (친구, 가족, 지인 등)
   - 기상 시간 / 수면 시간 / 나이
6. 숫자 질문 후보 (아래 중 3개를 무작위로 선택하되, 매번 다른 조합을 쓸 것):
   신발 사이즈, 가장 좋아하는 숫자, 오늘 걸음 수, 지갑 속 현금,
   형제자매 수, 마지막으로 먹은 음식의 대략적 칼로리, 현재 창밖 기온,
   최근 읽은 책/웹툰 페이지 수, 가장 최근에 산 물건 가격, 현재 배터리 잔량(%),
   지금 이 방에 있는 의자 수, 오늘 마신 물의 양(ml), 가장 오래 논 게임 플레이 시간(시간),
   지갑 속 카드 수, 집에 있는 식물 수, 냉장고에 달걀이 몇 개 남았는가,
   마지막으로 운동한 게 며칠 전인가, 현재 열린 탭 수, 책상 위 물건의 수,
   지금 입고 있는 옷에 주머니가 몇 개인가, 가장 자주 가는 편의점까지 걸음 수,
   지금 화면 밝기(%), 마지막으로 산 식료품 영수증 금액, 현재 시각의 분(分) 자리
7. 텍스트 질문 후보 (아래 중 2개를 무작위로 선택하되, 매번 다른 조합을 쓸 것):
   가장 싫어하는 색, 어릴 때 꿈, 마지막으로 한 거짓말의 내용,
   가장 두려운 것, 지금 기분을 날씨로 표현, 가장 오래된 기억,
   버리지 못하는 물건, 죽기 전에 꼭 하고 싶은 일,
   지금 당장 먹고 싶은 음식, 가장 후회되는 선택, 살면서 가장 운이 좋았던 순간,
   지금 이 순간 가장 갖고 싶은 것, 어린 시절 가장 무서웠던 것,
   누군가에게 말 못 한 비밀, 다시 태어난다면 바꾸고 싶은 것 한 가지,
   지금 가장 피하고 싶은 사람의 특징, 마지막으로 울었던 이유,
   자신을 동물에 비유하면, 지금 머릿속에 떠오르는 단어 하나,
   가장 자주 꾸는 꿈의 내용, 가장 소중한 물건을 잃어버린 경험,
   지금 당장 사라지고 싶은 곳, 자신의 가장 나쁜 습관,
   지금 이 순간 옆에 있었으면 하는 것
8. 말투: 신이 인간에게 묻는 고풍스럽고 위엄 있는 반말체로 작성할 것
   - "~인가?", "~하는가?", "~있는가?", "~되는가?", "~이더냐?", "~말하라", "~고하라" 같은 고전적 어미 사용
   - "~어?", "~야?", "~해?" 같은 현대 구어체 금지
   - 예시: "네 신발의 크기는 얼마인가?", "지금 이 순간 네 지갑 속에 현금이 얼마나 들어있는가?", "네가 가장 두려워하는 것을 고하라."
${KO_STYLE}
