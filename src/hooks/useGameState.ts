...
interface GameStore {
  ...
  markHintEventSeen: (id: string) => void;
}
...
export const useGameState = create<GameStore>()(
  persist(
    (set) => ({
      ...
      killPlayer: (cause) =>
        set((state) => ({
          ...
          meta: {
            ...
            legacyPoints: state.meta.legacyPoints + Math.max(1, Math.floor(state.run.depth / 3)),
      ...
      clearRun: () =>
        set((state) => ({
          meta: {
            ...
            bestDepth: Math.max(state.meta.bestDepth, state.run.depth),
            legacyPoints: state.meta.legacyPoints + 5,
            totalGhostWins: state.meta.totalGhostWins + state.run.ghostBattleWins,
          },
        })),
      ...
      markHintEventSeen: (id) =>
        set((state) => {
          if ((state.meta.seenHintEvents ?? []).includes(id)) return state;
          return { meta: { ...state.meta, seenHintEvents: [...(state.meta.seenHintEvents ?? []), id] } };
        }),
    }),
    {
